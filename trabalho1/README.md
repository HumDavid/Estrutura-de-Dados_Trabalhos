...
## Exercício: 2
```
Execute a seguinte s ́erie de opera ̧c ̃oes de pilha, assumindo uma pilha inicialmente vazia: push(5), push(3), pop(), push(2), push(8), pop(), pop(), push(9), push(1), pop(), push(7), push(6), pop(), pop(), push(4), pop(), pop().
```
## Exercício: 3
```
Implemente uma função com assinatura transfer(S, T) que transfira todos os elementos da pilha S para a pilha T, de modo que o elemento que come ̧ca no topo de S seja o primeiro a ser inserido em T, e o elemento na parte inferior de S termine no topo de T.
```
## Exercício: 4
```
...
## Exercício: 11
```
Crie um sistema usando uma pilha e uma fila para testar se uma determinada string  ́e um palíndromo (ou seja, se os caracteressão lidos da mesma forma, tanto para a frente quanto para trás).
```
## Exercício: 12
```
...
## Exercício: 20
```
Modifique a classe DoublyLinkedBase para incluir um método reverse que inverte a ordem da lista, mas sem criar ou destruir nenhum nó.
```